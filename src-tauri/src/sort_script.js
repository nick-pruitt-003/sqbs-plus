/*!
 * tablesort v5.7.1 (vendored) — MIT license
...
/* Alphanum sort extension — ported from qb-stats-plus */
(function () {
  var reA = /[^a-zA-Z]/g;
  var reN = /[^-.\d]/g;
  Tablesort.extend('alphanum', function () { return true; }, function (a, b) {
...
/* Initialize sorting on tables marked with data-sort attribute */
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('table[data-sort]').forEach(function (table) {
    var mode = table.getAttribute('data-sort');
    if (mode === 'none') return;

    table.querySelectorAll('thead > tr > th').forEach(function (th) {
      th.setAttribute('data-sort-method', 'alphanum');
    });

    if (mode === 'rerankFirst') {
      var first = table.querySelector('thead > tr > th');
      if (first) {
        first.setAttribute('data-sort-method', 'none');
        first.textContent = '#';
      }
    }
